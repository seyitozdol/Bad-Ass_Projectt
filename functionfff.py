...
def df_general_stats(df, show_print=False):
    ...
    # count of users
    n_users = df[
        "user"].nunique()  # number of people that messaged the group (including those who left the group) (-1 for group notification)
    # list of users
...
def most_active_weekdays(df):
    ...
    # PLOT: grouped by hour
    sns.barplot(grouped_by_dayt.day_t, grouped_by_dayt.message_count, palette="bright",
...
def day_hour_heatmap(df):
    ...
    pt = grouped_by_month_and_day.pivot_table(index='day_t', columns='hour', values='message_count').reindex(index=days,
                                                                                                             columns=hours)
